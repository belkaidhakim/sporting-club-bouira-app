...
export default function AthletePerformancesModal({ athlete, onClose }) {
  ...
  // Formulaire de saisie d'un chrono
  ...
  // Charger l'historique des performances de l'athlète
  useEffect(() => {
    if (!athlete?.id) return;

    const storageKey = `${PERFORMANCES_STORAGE_PREFIX}${athlete.id}`;
    const localData = localStorage.getItem(storageKey);
    if (localData) {
      try {
        setPerformances(JSON.parse(localData));
      } catch {}
    }

    // Essayer également de synchroniser avec Supabase si une colonne ou table existe
    const fetchRemote = async () => {
      try {
        const { data } = await supabase
          .from('athletes')
          .select('observations_medicales')
          .eq('id', athlete.id)
          .single();
        // Fallback silencieux
      } catch {}
  ...
  // Performances pour l'épreuve actuellement sélectionnée
  ...
  // Meilleur temps personnel (Record / Personal Best) sur l'épreuve
  const personalBest = useMemo(() => {
    if (currentEventPerfs.length === 0) return null;
    return [...currentEventPerfs].sort((a, b) => a.seconds - b.seconds)[0];
  }, [currentEventPerfs]);

  // Données formatées pour le graphique de progression
    ...
    <div style={{
      ...
      <div className="glass-panel" style={{
        ...
        <div className="p-5 overflow-y-auto" style={{ flex: 1 }}>
          {/* FORMULAIRE D'AJOUT D'UN CHRONO */}
          {showAddForm && (
            <div className="p-4 rounded-xl mb-6" style={{ backgroundColor: 'rgba(56, 189, 248, 0.06)', border: '1px solid rgba(56, 189, 248, 0.25)' }}>
              <h3 style={{ fontSize: '0.95rem', fontWeight: 800, color: '#38bdf8', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <Timer size={17} /> Enregistrer un temps de référence
              </h3>
              <form onSubmit={handleAddPerformance}>
                ...
                <div className="flex justify-end gap-2 mt-2">
                  ...
                  <button 
                    ...
                    style={{ padding: '0.35rem 1rem', display: 'inline-flex', alignItems: 'center', gap: '5px' }}
                  >
          ...
          {/* KPI ET RECORD PERSONNEL SUR L'ÉPREUVE */}
          ...
          {chartData.length > 1 && (
            <div className="p-4 rounded-xl mb-5" style={{ backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border-color)' }}>
              <h4 style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                Évolution Chronométrique (Objectif : Baisser le temps ⏱️)
              </h4>
          ...
          {/* TABLEAU DES CHRONOS ENREGISTRÉS */}
          <div>
            <h4 style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.6rem' }}>
              Historique des Chronos ({currentEventPerfs.length})
            </h4>
            {currentEventPerfs.length === 0 ? (
              <div className="text-center text-muted text-xs py-8" style={{ backgroundColor: 'var(--bg-tertiary)', borderRadius: '12px' }}>
                Aucun temps enregistré pour cette épreuve. Cliquez sur <strong>"+ Nouveau Chrono"</strong> pour ajouter le premier temps.
              </div>
            ) : (
